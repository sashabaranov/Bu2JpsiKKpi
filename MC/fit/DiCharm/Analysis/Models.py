...
class Charm2_pdf (object):
    ...
    def __init__(self,
        ...
        self.bkg1 = Bkg_pdf('Bkg(1)' + suffix, self.m1)
        self.bkg2 = Bkg_pdf('Bkg(2)' + suffix, self.m2)

        self.bkgA = Bkg_pdf('Bkg(A)' + siffix, self.m1)
        ...
        self.bb_pdf = ROOT.RooProdPdf(
            "B1B2pdf", "Bkg(A) x Bkg(B)", self.bkgA.pdf, self.bkgB.pdf)
...
class BW_pdf(object):
    ...
    def __init__(self,
        ...
        # BreitWigner::BreitWigner(   char* name,
                                    # char* title,
                                    # RooAbsReal& x,
                                    # RooAbsReal& mass,
                                    # RooAbsReal& width,
...
class PhaseSpace_pdf(object):
    ...
    def __init__(self, name, x, low, high, N, L, power=0):
        ...
        phis_default = [-0.9552979324772402, 0.8178354630178042]
        self.poly_arglist = ROOT.RooArgList()
        self.phis = [ROOT.RooRealVar('phi%d_%s' % (i, name), 'phi%d(%s)' % (i, name), phis_default[i], -3.5, 3.5)
